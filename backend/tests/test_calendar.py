...
def test_create_event_linked_to_task(client: TestClient):
    ...
    task_res = client.post(
        "/tasks/", json={"title": "Prepare presentation"}, headers=headers
    )
...
def test_update_and_delete_event(client: TestClient):
    ...
    update_response = client.put(
        f"/calendar/events/{event_id}", json=update_data, headers=headers
    )
...
def test_get_calendar_view(client: TestClient):
    ...
    response = client.get(
        f"/calendar/?start_date={start_date}&end_date={end_date}", headers=headers
    )
