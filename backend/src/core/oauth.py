...
def load_and_register_providers():
    ...
    with Session(engine) as session:
        statement = select(OAuthProviderConfig).where(OAuthProviderConfig.is_active == True)
        ...
        for provider_config in active_providers:
            ...
            if not decrypted_secret:
                print(f"ERROR: Could not decrypt secret for provider '{provider_config.provider}'. Skipping.")
                continue
