...
async def auth_callback(request: Request, provider: str, db: Session = Depends(get_db)):
    ...
    oauth_account = db.exec(select(OAuthAccount).where(
        OAuthAccount.provider == provider, OAuthAccount.account_id == provider_account_id
    )).first()
    ...
    if oauth_account:
        ...
        new_oauth_account = OAuthAccount(provider=provider, account_id=provider_account_id, user_id=user.id)
...
async def register(user_data: UserCreate, db: Session = Depends(get_db)) -> UserPublic:
    if db.exec(select(User).where(or_(User.username == user_data.username, User.email == user_data.email))).first():
        raise HTTPException(
            status_code=409,
...
async def get_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    user = db.exec(select(User).where(or_(User.username == form_data.username, User.email == form_data.username))).first()
    if not user:
        raise HTTPException(
            401,
            'username/email or password are incorrect'
        )
