...
async def get_user_folders(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    folders = current_user.idea_folders
    return folders

@folders_router.put('/{folder_id}', response_model=FolderPublic)
async def update_folder(folder_id: UUID, folder_in: FolderUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def delete_event(folder_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    folder = db.get(IdeaFolder, folder_id)
    if not folder or folder.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="IdeaFolder not found")
    db.delete(folder)
