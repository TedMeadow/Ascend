...
async def create_idea(idea_in: IdeaCreate, background_tasks: BackgroundTasks, current_user: User = Depends(get_current_user), db: Session = Depends(get_db), ):
    folder = db.get(IdeaFolder, idea_in.folder_id)
    if not folder or folder.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Cannot create Idea in this folder")
    tags_to_assign = []
    for tag_name in idea_in.tags:
        tag_statement = select(Tag).where(Tag.name == tag_name, Tag.owner_id == current_user.id)
        tag = db.exec(tag_statement).first()
        if not tag:
            tag = Tag(name=tag_name, owner_id=current_user.id)
            db.add(tag)
        tags_to_assign.append(tag)
    idea = Idea.model_validate(idea_in, update={'owner_id': current_user.id, 'tags': tags_to_assign})
    if idea.idea_type == IdeaType.LINK:
        background_tasks.add_task(fetch_and_save_metadata, idea_id=idea.id, url=idea.url)
    db.add(idea)
    db.commit()
    db.refresh(idea)
    return idea
...
def get_ideas(
    ...
    if q:
        ...
        statement = statement.where(or_(
            ...
            Idea.content.ilike(search_query)
        ))
...
def get_idea(idea_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def update_idea(idea_id: UUID, idea_in: IdeaUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if "tags" in update_data:
        ...
        for tag_name in tag_names:
            tag = db.exec(select(Tag).where(Tag.name == tag_name, Tag.owner_id == current_user.id)).first()
            if not tag:
                tag = Tag(name=tag_name, owner_id=current_user.id)
            tags_to_assign.append(tag)
...
def delete_idea(idea_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Удаляет идею."""
...
@ideas_router.post("/{idea_id}/promote-to-task", response_model=TaskPublic, status_code=status.HTTP_201_CREATED)
def promote_idea_to_task(
    ...
    if idea.generated_task_id:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This idea has already been promoted to a task")
